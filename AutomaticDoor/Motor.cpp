...
void Motor::rotate(bool rotdirection, float centimeter_per_sec, float movingdistance, bool sensorcheck){
  ...
  if(((100*(int)movingdistance) % (100*(int)rounddown))/100.0 < rounddown/2.0){
    this->movingdistance = movingdistance - ((100*(int)movingdistance) % (100*(int)rounddown))/100.0;
  }
  else{
    this->movingdistance = movingdistance + ((100*(int)movingdistance) % (100*(int)rounddown))/100.0;
  }
  ...
  long feedback_sum = 0;
  long feedback_expected = steps/(rounddown*steppercm);
  ...
  if(feedback_differ != 0){
    ...
    if(feedback_expected < feedback_sum){
      digitalWrite(rotdirection_pin,-1*(this->rotdirection));
    }
    for(int j = 0; j < feedback_differ;j++){
      digitalWrite(intopulse_pin,HIGH);
      delayMicroseconds(delaytime);
      digitalWrite(intopulse_pin,LOW);
      delayMicroseconds(delaytime);
